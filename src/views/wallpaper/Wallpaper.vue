...
import { Button as TinyButton,ButtonGroup as TinyButtonGroup,Numeric as TinyNumeric } from "@opentiny/vue";
...
onMounted(async () => {
  // 创建用户图片文件夹
  await mkdir("skydesk", { baseDir: BaseDirectory.Picture, recursive: true });
  await mkdir("wallpaper", { baseDir: BaseDirectory.AppData, recursive: true });
});
...
const savewallpaper = async function (path: string) {
  ...
  console.log("skydesk\\" + filename,arr)
  await writeFile(
    "skydesk\\" + filename,
    new Uint8Array(arr),
...
const showsetting = function () {
  ...
  // 模拟右击事件
  let body = document.getElementById("setting");
  if(!body) return
